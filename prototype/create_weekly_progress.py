...
c = ws1['A2']
c.value = "M1组（杨胜威、宋丽娜）→ 模块一 签到情绪    |    M2组（师润佳、郑永涛）→ 模块二 脑力游戏"
...
m2_tasks = [
    # 师润佳 — 后端
    ("M2-01", "脑力游戏", "师润佳", "需求评审 + 确认game_records表结构 + 难度自适应引擎方案设计", "后端", "周一 8/3", "1天", "未开始"),
    ("M2-02", "脑力游戏", "师润佳", "GameRecord Entity/Mapper + 游戏记录写入接口 POST /api/game/record", "后端", "周二 8/4", "1天", "未开始"),
    ("M2-03", "脑力游戏", "师润佳", "难度自适应算法实现(正确率≥80%升级/≤40%降级) + 查询用户当前难度", "后端", "周三 8/5", "1天", "未开始"),
    ("M2-04", "脑力游戏", "师润佳", "游戏成绩查询接口 + 跨域训练分布统计 + 放弃率埋点", "后端", "周四 8/6", "1天", "未开始"),
    ("M2-05", "脑力游戏", "师润佳", "Postman接口测试 + 修bug + 整理接口文档给前端", "后端", "周五 8/7", "1天", "未开始"),
    # 郑永涛 — 前端
    ("M2-06", "脑力游戏", "郑永涛", "需求评审 + 研究小程序Canvas/动画方案 + 研读原型设计稿", "前端", "周一 8/3", "1天", "未开始"),
    ("M2-07", "脑力游戏", "郑永涛", "游戏列表页 — 4款游戏卡片展示(记忆/注意/推理/语言) + 难度等级显示", "前端", "周二 8/4", "1天", "未开始"),
    ("M2-08", "脑力游戏", "郑永涛", "G1翻牌配对游戏 — 难度2×2→4×4 + 翻错不扣分提示 + 完成星级", "前端", "周三 8/5", "1天", "未开始"),
    ("M2-09", "脑力游戏", "郑永涛", "G2找不同游戏 — 两图对比 + 自适应干扰物 + 重准确不抢速", "前端", "周四 8/6", "1天", "未开始"),
    ("M2-10", "脑力游戏", "郑永涛", "G3找规律 + G4看图说词 初步开发 + 对接后端记录写入接口", "前端", "周五 8/7", "1天", "未开始"),
]
...
c = ws3['A1']
c.value = "M2 脑力游戏组 — 师润佳(后端) / 郑永涛(前端)"
c.font = Font(name='微软雅黑', bold=True, size=14, color=C_HEADER_F)
c.fill = PatternFill('solid', fgColor=C_HEADER)
c.alignment = Alignment(horizontal='center', vertical='center')
...
for col, h in enumerate(headers2, 1):
    # 替换人名
    h = h.replace("杨胜威（后端）", "师润佳（后端）").replace("宋丽娜（前端）", "郑永涛（前端）")
...
acceptance = [
    ...
    # M2
    ("M2游戏", "G1翻牌配对", "2×2起→4×4，翻错不扣分给提示，完成给星级", "郑永涛", "待验收"),
    ("M2游戏", "G2找不同", "两图找不同，自适应干扰物，重准确不抢速", "郑永涛", "待验收"),
    ("M2游戏", "G3找规律", "续写数列/物品归类，初步可玩", "郑永涛", "待验收"),
    ("M2游戏", "G4看图说词", "初步可玩", "郑永涛", "待验收"),
    ("M2游戏", "难度自适应", "≥80%升级/≤40%降级/中间维持，每局结束返回新难度", "师润佳", "待验收"),
    ("M2游戏", "游戏记录写入", "正确率/用时/完成等级写入DB", "师润佳", "待验收"),
    ("M2游戏", "训练能力展示", "每局结束展示'今天练了记忆力/注意力/推理力/语言力'", "师润佳+郑永涛", "待验收"),
    ("M2游戏", "无挫败反馈", "错误操作不产生挫败式提示，只鼓励", "郑永涛", "待验收"),
    ("M2游戏", "单局≤5分钟", "每局3-5分钟内可完成", "郑永涛", "待验收"),
]
