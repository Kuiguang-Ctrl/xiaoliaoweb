...
public interface M2GameService {
     ...
     */
    List<GameListVO> listGames(Long userId);

    /**
     * 开始一局：按最近完成局的 next_level_no 选难度，创建进行中记录
     */
    GameStartVO start(Long userId, Long gameId);

    /**
     * 结束一局：计算正确率与星级，自适应升降级，写日汇总与成就
     */
    GameFinishResultVO finish(Long userId, GameFinishVO vo);

    /**
     * 查询某日训练汇总（默认今天）
     */
    DailyTrainVO dailyTrain(Long userId, String dateStr);
}
