...
public class ChatController {
    ...
    @Operation(summary = "AI 对话", description = "发送用户消息，返回 AI 回复文本、意图、质检结果与意图推荐。userId 由鉴权自动带入，无需传")
    @PostMapping
    public Result<Map<String, Object>> chat(
            @Parameter(hidden = true) @RequestAttribute(AuthContext.USER_ID_ATTR) String userId,
            @RequestBody ChatRequest request) {
        ChatResponse resp = aiService.chat(userId, request.getMessage(), request.getConversationHistory(), request.getSessionId());
